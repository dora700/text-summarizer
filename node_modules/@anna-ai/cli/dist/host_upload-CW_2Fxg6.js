...
var HostUploadBridge = class {
	...
	mockCall(params) {
		...
		if (mode === "inline") {
			...
			const sizeBytes = Math.floor(b64.length * .75);
			return {
				download_url: url,
				url,
				r2_key: r2Key,
		...
		if (mode === "confirm") {
			...
			const url = `https://mock.local/${key}`;
			return {
				download_url: url,
				url,
				r2_key: key,
	...
	async realCall(params) {
		...
		return normalizeUploadResult(await res.json());
	}
};
/**
* Backfill canonical field aliases on a `host/uploadFile` result.
* Older nexus builds emit `url` / `bytes` / `expires_in` while the SDK
* contract is `download_url` / `size_bytes` / `expires_at` (forum #168).
* Never overwrites existing fields.
*/
function normalizeUploadResult(result) {
	if (!result || typeof result !== "object") return result;
	if (result.url != null && result.download_url == null) result.download_url = result.url;
	else if (result.download_url != null && result.url == null) result.url = result.download_url;
	if (result.bytes != null && result.size_bytes == null) result.size_bytes = result.bytes;
	else if (result.size_bytes != null && result.bytes == null) result.bytes = result.size_bytes;
	if (result.expires_in != null && result.expires_at == null) result.expires_at = new Date(Date.now() + Number(result.expires_in) * 1e3).toISOString();
	return result;
}
