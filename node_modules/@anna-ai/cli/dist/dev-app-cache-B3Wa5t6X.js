...
/**
* Bump this whenever the server-side dev-app grant shape changes in a way
* that requires a re-register to pick up (registration is idempotent and
* refreshes the grant bundle server-side). Cached entries written with a
* lower `cache_version` are treated as stale and re-POSTed.
*
*   1 — initial (implicit: entries without `cache_version`; server wrote
*       `llm_grant` only)
...
async function ensureDevAppRegistered(args) {
	...
	if (cached && cached.host === canonical && cached.slug === args.input.slug && cached.cache_version >= DEV_APP_CACHE_VERSION) try {
	...
	const entry = {
		...
		cache_version: DEV_APP_CACHE_VERSION
	};
